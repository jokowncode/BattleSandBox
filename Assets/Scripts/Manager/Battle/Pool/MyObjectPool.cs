...
public class MyObjectPool<T> : IDisposable, IObjectPool<T> where T : class {
    ...
    public void RemoveNullObj() {
      int preCount = this.CountInactive;
      this.m_List.RemoveAll((obj) => obj == null);
      int diff = preCount - this.CountInactive;
      this.CountAll -= diff;
    }
